...
execute if score #xh_bool xh_find matches 1 at @s run playsound block.glass.hit block @a ~ ~ ~ 0.7 1
execute if score #xh_bool xh_find matches 1 at @s run summon item ~ ~0.5 ~ {Item:{id:"minecraft:vex_spawn_egg",count:1,components:{"minecraft:max_stack_size":3,"minecraft:item_name":'"Кальян"',"minecraft:custom_model_data":228001,"minecraft:entity_data":{id:"minecraft:marker",Tags:["xh","xh_place_marker"]}}}}
